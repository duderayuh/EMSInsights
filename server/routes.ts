...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/health", async (req, res) => {
    try {
      ...
      const audioStatus = audioProcessor ? audioProcessor.getStatus() : { status: 'unavailable' };
      const transcriptionStatus = transcriptionService ? transcriptionService.getStatus() : { status: 'unavailable' };
      const nlpStats = nlpClassifier && typeof nlpClassifier.getKeywordStats === 'function' 
        ? nlpClassifier.getKeywordStats() 
        : { keywords: 0, classifications: 0 };
  ...
  app.get("/api/config/google-maps-key", async (req, res) => {
    try {
      // Try Google Maps API key first, then fall back to Address Validation key
      const apiKey = process.env.GOOGLE_MAPS_API_KEY || process.env.GOOGLE_ADDRESS_VALIDATION_API_KEY;
      if (!apiKey) {
  ...
  app.post('/api/alerts', requireAuth, async (req, res) => {
    try {
      ...
      if (wsService) { wsService.broadcastAlert(alert); };
  ...
  app.patch('/api/alerts/:id/read', requireAuth, async (req, res) => {
    try {
      ...
      if (wsService) { wsService.broadcastAlertUpdate(alert); };
    ...
    try {
      ...
      if (transcriptionResult) {
        ...
        if (wsService) {
          ...
          // Update system stats
          const stats = await storage.getCurrentStats();
          if (wsService) { wsService.broadcastStatsUpdate(stats); };
        }
      }
    } catch (error) {
